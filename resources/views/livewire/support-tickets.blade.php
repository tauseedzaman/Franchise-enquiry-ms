<main>
    ...
    <div class="container">
        <div class="row  mt-4 justify-content-around">
            <div class="col ">
                <h1 class=""><b>My Tickets</b></h1>
            </div>
            <div class="col float-right text-right ">
                <button wire:click="show_create_ticket_form()" class="btn btn-primary" type="button"><b>New
                        Ticket</b></button>
            </div>
        </div>
                    ...
                    <table class="table table-light ">
                        ...
                        <tbody>
                            ...
                            <tr  class="cursor-pointer" style="cursor: pointer;" id="needHover">
                                    <td><a class="nav-link text-dark" href="{{ route("auth.viewTicket",$ticket->uuid) }}">{{ $loop->index }}</a></td>
                                    <td><a class="nav-link text-dark" href="{{ route("auth.viewTicket",$ticket->uuid) }}">{{ $ticket->subject }}</a></td>
                                    <td><a class="nav-link text-dark" href="{{ route("auth.viewTicket",$ticket->uuid) }}">{{ $ticket->created_at->format("d /M /Y h:m") }}</a></td>
                                    <td><a class="nav-link text-dark" href="{{ route("auth.viewTicket",$ticket->uuid) }}"> {{ $ticket->updated_at }} </a></td>
                                    <td><a class="nav-link text-dark" href="{{ route("auth.viewTicket",$ticket->uuid) }}">{{ $ticket->status->name }}</a></td>
                                </tr>
                            </a>
                            @endforeach
                        </tbody>
